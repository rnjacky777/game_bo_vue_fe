<template>
  <div v-if="show" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 backdrop-blur-sm">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 space-y-6 relative border border-gray-200">
      ...
      <!-- 標題 -->
      <h3 class="text-xl font-semibold text-gray-800 text-center">編輯條件</h3>
      ...
      <div class="max-h-[60vh] overflow-y-auto space-y-4 pr-2 custom-scrollbar">
        <div v-for="(story, index) in localModelValue" :key="index"
          ...
          <!-- 名稱輸入 -->
          <div>
            <label class="block text-sm text-gray-500 mb-1">條件類型</label>
            <select v-model="story.condition_key"
              ...
              <option disabled value="">請選擇條件</option>
              <option v-for="option in conditionOptions" :key="option.value" :value="option.value">
                {{ option.label }}
              </option>
            </select>

            <label class="block text-sm text-gray-500 mb-1">條件值</label>
            <input v-model="story.condition_value" type="text"
...
const addStory = () => {
  localModelValue.value.push({ condition_key: '', condition_value: '' });
};
