    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 space-y-6 relative border border-gray-200">
      ...
      <div class="max-h-[60vh] overflow-y-auto space-y-4 pr-2 custom-scrollbar">
        <div v-for="(effect, index) in localModelValue" :key="index"
          ...
          <!-- 效果類型 -->
          <div>
            <label class="block text-sm text-gray-500 mb-1">效果類型</label>
            <select v-model="effect.status_effect_key"
              class="w-full border border-gray-300 rounded-lg px-3 py-2 mb-2 focus:outline-none focus:ring-2 focus:ring-blue-400 transition">
              <option disabled value="">請選擇效果</option>
              <option v-for="option in statusEffectOptions" :key="option.value" :value="option.value">
                {{ option.label }}
              </option>
            </select>
            ...
            <input v-model="effect.status_effect_value" type="text"
      ...
      <div class="flex justify-center">
        <button @click="addStatusEffect" type="button"
...
const statusEffectOptions = [
  { value: 'increase_hp', label: '增加HP' },
  { value: 'decrease_hp', label: '減少HP' },
  { value: 'poison', label: '中毒' },
  { value: 'burn', label: '灼燒' },
  { value: 'stun', label: '昏迷' },
  { value: 'freeze', label: '冰凍' },
  // 可以根據你的實際狀態效果再增加
];
...
const addStatusEffect = () => {
  localModelValue.value.push({ status_effect_key: '', status_effect_value: '' });
};
